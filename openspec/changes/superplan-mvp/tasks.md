...
- [x] 1.6 冒烟:`pi -e ./src/index.ts` 下创建→写入→恢复全链路手动跑通(用户确认此前已手动验证)
- [x] 2.1 实现 `ctx.ui` TUI 选型路径(候选展示/选择/备注),验证无 deck 环境下决策完整写入 decisions.md(单测)
- [x] 2.2 实现 deck 检测(`getAllTools`)与缺失时确认安装/拒绝退化分支,验证拒绝安装时退化 TUI 且 timeline 记录原因(单测)
- [x] 2.3 实现 DecisionPoint→deck JSON 转换与 sendMessage 指令生成,验证输出 JSON 符合 deck-schema(单测)
- [x] 2.4 实测一次真实 deck 调用,确认 `tool_result` 中 `details.selections` 结构;结果记录到 design.md Open Questions(若需解析文本则补解析器)(实测:details 完整保留 selections=Record<slideId,label>,无需文本解析器)
- [x] 2.5 实现 `tool_result` 捕获 + 强制回写 decisions.md(含未采纳理由/revision),验证"有选择无落盘"被拦截补写(单测)
...
- [x] 4.1 实现任务状态机(待办/进行/完成/失败)+ 状态落盘 + timeline,验证状态变更重启后可读回(单测)
- [x] 4.2 实现暂停/恢复命令,验证新会话从磁盘定位下一个待办任务(单测)
- [x] 4.3 实现 session_before_compact 快照落盘,验证压缩钩子触发时任务状态先写盘(单测)
- [x] 4.4 冒烟:上下文压缩后继续执行,无进度丢失(实测:28206 tokens 压缩,compact-snapshot 落盘 timeline,压缩后 resume 定位 S1)

## 5. M5 模型配置与提问桥

- [x] 5.1 实现阶段模型配置读写(README.md 中 planning/execution/review + thinking level),验证非法模型/未认证/思考级别不兼容 fail-closed(单测)
- [x] 5.2 实现手动升降档命令(setModel + 校验 + timeline 记录),验证跨供应商时给出新会话交接提示(单测)
- [x] 5.3 实现 plan_mode_question 检测与可选桥接,验证不可用时 ctx.ui 路径产出一致的内部答案模型(单测)
